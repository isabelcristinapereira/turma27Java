...
@CrossOrigin(origins = "*", allowedHeaders = "*") //vai aceitar qualquer origem
public class UsuarioController {
	...
	return usuarioService.atualizarUsuario(usuario).map(resp -> ResponseEntity.status(HttpStatus.OK)
	.body(resp)).orElse(ResponseEntity.status(HttpStatus.BAD_REQUEST).build());
	}
